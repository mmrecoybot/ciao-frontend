...
export default async function AddPage({ searchParams, params: { lang } }) {
  ...
  try {
    ...
    // );
    const selectedCategoryProducts = searchParams.category
      ? products.filter(
          (product) => product?.category?.name === searchParams.category,
        )
      : products;
    ...
    const filteredProducts = selectedCategoryProducts
      .filter((product) => {
        const searchProducts = searchParams?.search
          ? product?.name
              ?.toLowerCase()
              .includes(searchParams?.search.toLowerCase())
          : true;
        const subProducts = searchParams?.sub
          ? product?.subCategory?.name === searchParams?.sub
          : true;
        const brandProducts = searchParams?.brand
          ? product?.brand?.name === searchParams?.brand
          : true;
