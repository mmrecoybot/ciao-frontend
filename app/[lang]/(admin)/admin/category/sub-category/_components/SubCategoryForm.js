...
import { useAddSubCategoryMutation, useUpdateSubCategoryMutation } from "@/store/slices/subCategoryApi";
import { useEffect } from "react";
import { useForm } from "react-hook-form";
import { toast } from "react-toastify";

const SubCategoryForm = ({ subCategory, isEdit, setShowForm, dictionary }) => {
  ...
  // const [logoPreview, setLogoPreview] = useState(subCategory?.logo);
  ...
  useEffect(() => {
    ...
    if (addError) {
      ...
      toast.error(
        `Failed to update brand: ${updateError.message || "Unknown error occurred"
        }`
      );
    }
  ...
  const onSubmit = async (data) => {
    ...
    if (isEdit) {
      ...
      updateSubCategory({ ...data, id: subCategory?.id });
    } else {
      addSubCategory({ ...data, "logo": "" });
    }
  };
    ...
    >
      ...
      <div>
        ...
        <input
          id="name"
          type="text"
          placeholder={dictionary.categoryPages.sub_category_name}
      ...
      <div>
        ...
        <select
          ...
          defaultValue={1}
        >
          <option value="">{dictionary.categoryPages.select_category}</option>
      ...
      <button
        ...
        className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-black dark:text-gray-400 bg-emerald-300 dark:bg-emerald-800 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500"
      >
